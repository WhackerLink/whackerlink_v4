...
namespace WhackerLinkServer.RestApi.Controllers
{
    [ApiController]
    [Route("api/masters")]
    public class UpdateMasterController : ControllerBase
    {
        [HttpPost("{masterName}/update")]
        public IActionResult UpdateMaster([FromBody] JsonElement masterConfigJson)
        {
            try
            {
                ...
                bool success = Program.UpdateMaster(masterConfig.Name, masterConfig);
                if (!success)
                {
                    return StatusCode(500, new { error = "Failed to update the master configuration." });
                }
